...
compute_hamming_dists <- function(aln, mode, seq_info){
  # calculate pairwise genetic distances (normalized hamming, substitutions/site) and convert to long format
  dist.dna(aln, model = "raw", pairwise.deletion = TRUE, as.matrix = TRUE) %>%
    graph.adjacency(mode = mode, weighted = TRUE) %>% 
    get.data.frame() %>% 
    rename(seq1 = from, seq2 = to, dist = weight) %>% 
